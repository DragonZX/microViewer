...
class Process:
    ...
    def kill(proc_list):
        for proc in proc_list:
            ExternalApp.run(f"taskkill /F /T /IM {proc}")
...
class MvApp:
    ...
    @staticmethod
    def run_tunnel(srv_port, key, mode, tunnel_local_port, tunnel_remote_port, user, run_type):
    ...
    def service_on_rd():
        ...
        if Check.admin_right():
            ...
            if MV.connection_type == "id":
                ExternalApp.run(f'schtasks /create /xml "{k_task}" /tn "Microsoft\Windows\microViewer\k_tunnel"')
                # Экранируем путь, чтобы в пути после пробела оставшаяся часть не воспринималась как аргумент;
                ExternalApp.run(f'schtasks /change /tn "Microsoft\Windows\microViewer\k_tunnel" /tr "\'{k_service}\'"')
                ExternalApp.run(f'schtasks /run /tn "Microsoft\Windows\microViewer\k_tunnel"')
            # Включение планов электропитания;
            if Service.disable_sleep_mode == "true":
                ExternalApp.run(f'powercfg -Change -standby-timeout-ac 0')
                ExternalApp.run(f'powercfg -Change -standby-timeout-dc 120')
            if Service.disable_hibernation_mode == "true":
                ExternalApp.run(f'powercfg -Change -hibernate-timeout-ac 0')
                ExternalApp.run(f'powercfg -Change -hibernate-timeout-dc 120')
            if Service.disable_lock_screen == "true":
                ExternalApp.run(f'powercfg -Change -monitor-timeout-ac 0')
                ExternalApp.run(f'powercfg -Change -monitor-timeout-dc 10')
            return True
        else:
            return False

    @staticmethod
    def service_remove_rd():
        # Для выполнения, необходимы права Администратора;
        if Check.admin_right():
            ...
            if MV.connection_type == "id":
                ...
                # Завершаем процессы;
                Process.kill_pid(Pids.tun_service)
                Process.kill(["kitty_service.exe"])
                # Отключаем параметр в конфиге;
                edit_config(mv_conf_path, "Pids", "tun_service", "none")
            # Отключаем параметры в конфиге;
            edit_config(mv_conf_path, "Service", "enabled", "false")
            # Т.к. задействована фиксация зашифрованного пароля;
            # edit_config(mv_conf_path, "Service", "id", "")
            # edit_config(mv_conf_path, "Service", "password", "")
            edit_config(mv_conf_path, "Modes", "service", "false")
            # Отключение планов электропитания;
            if Service.disable_sleep_mode == "true":
                ExternalApp.run(f'powercfg -Change -standby-timeout-ac 0')
                ExternalApp.run(f'powercfg -Change -standby-timeout-dc 30')
            if Service.disable_hibernation_mode == "true":
                ExternalApp.run(f'powercfg -Change -hibernate-timeout-ac 0')
                ExternalApp.run(f'powercfg -Change -hibernate-timeout-dc 30')
            if Service.disable_lock_screen == "true":
                ExternalApp.run(f'powercfg -Change -monitor-timeout-ac 15')
                ExternalApp.run(f'powercfg -Change -monitor-timeout-dc 10')
            return True
        else:
            return False
...
class Tool:
    ...
    @staticmethod
    def save_parent_window_pos(parent_window):
    ...
    @staticmethod
    def show_hide_service_fix_id_pw():
        if form2.checkBox_id_pass_enable.isChecked():
...
def gui_settings_applay():
    ...
    if not modes_client and not modes_server and Modes.service != "true":
        ...
        current_values = {
            ...
            'service_fix_id_pw': str(form2.checkBox_id_pass_enable.isChecked()).lower(),
            'service_id': form2.spinBox_id.value(),
            'service_password': form2.lineEdit_password.text(),
            'service_disable_sleep_mode': str(form2.checkBox_standby.isChecked()).lower(),
            'service_disable_hibernation_mode': str(form2.checkBox_hibernate.isChecked()).lower(),
            'service_disable_lock_screen': str(form2.checkBox_monitor.isChecked()).lower()
        }
        # Если изменилось значение, то обновляем в конфиге на новое;
        ...
        if current_values["service_password"] != Service.password:
            ...
            form.label_password_data.setText(gen_pw())
        if current_values["service_disable_sleep_mode"] != Service.disable_sleep_mode:
            edit_config(mv_conf_path, "Service", "disable_sleep_mode",
                        (str(current_values["service_disable_sleep_mode"])).lower())
        if current_values["service_disable_hibernation_mode"] != Service.disable_hibernation_mode:
            edit_config(mv_conf_path, "Service", "disable_hibernation_mode",
                        (str(current_values["service_disable_hibernation_mode"])).lower())
        if current_values["service_disable_lock_screen"] != Service.disable_lock_screen:
            edit_config(mv_conf_path, "Service", "disable_lock_screen",
                        (str(current_values["service_disable_lock_screen"])).lower())
...
class HandlerModeButton:
    ...
    @staticmethod
    def ab_connect_disconnect_row_doubleclick():
        if not modes_client:
            ab_connect_line = ab_insert_data_connect()
            form.lineEdit_id.setText(ab_connect_line[1])
            form.lineEdit_password.setText(ab_connect_line[2])
...
def set_advanced_settings_forms():
    ...
    Tool.check_box(form2.checkBox_id_pass_enable, Service.fix_id_pw)
    Tool.show_hide_service_fix_id_pw()
    form2.checkBox_id_pass_enable.stateChanged.connect(Tool.show_hide_service_fix_id_pw)
    Tool.check_box(form2.checkBox_standby, Service.disable_sleep_mode)
    Tool.check_box(form2.checkBox_hibernate, Service.disable_hibernation_mode)
    Tool.check_box(form2.checkBox_monitor, Service.disable_lock_screen)
